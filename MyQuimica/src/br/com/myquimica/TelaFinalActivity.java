...
public class TelaFinalActivity extends BaseGameActivity {
	...
	private BitmapTextureAtlas fonteRankingTexture;
	private Font fonteRanking;
	...
	protected void onCreate(Bundle pSavedInstanceState) {
		...
		this.pontos = extras.getInt("pontos");
	...
	public void onLoadResources() {
		...
		this.fonteRankingTexture = new BitmapTextureAtlas(1024, 1024,TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		this.fonteRanking = FontFactory.createFromAsset(this.fonteRankingTexture,this, "font/Archive.otf", 35, true, Color.rgb(0,130,238));
		...
		this.mEngine.getTextureManager().loadTextures(this.cenarioTexture,this.fonteTexture,
				this.fontePontosTexture, this.fonteMensagemTexture,this.botaoJogarNovamenteTexture,this.botaoSairTexture,this.fonteRankingTexture);
		this.mEngine.getFontManager().loadFonts(this.fontScore,this.fontPontos, this.fontMensagem, this.fonteRanking);
	}
	...
	public Scene onLoadScene() {
		...
		Text textoFormulaQuimica = new Text(250, 10,this.fontScore, "PARAB�NS");
		Text textoPontos = new Text(210, 250, this.fontPontos,"Voc� fez: " +pontos+ " Pontos");
		Text textoMensagem= new Text(130, 430, this.fontMensagem,"O estudo � a base para o");
...
//			startActivity(intent1);
	...
	private void getNomeUsuario(){
		...
		alert.setPositiveButton("Salvar", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				nomeJogador = nome.toString();
				}
				salvarJogador(nomeJogador, pontos);
	...
	private void exibirRanking(){
		int eixoY = 100;
		List<Jogador> jogadores = FController.getInstance().buscarListaJogadores(getApplicationContext());
		Collections.sort(jogadores, new Comparator<Jogador>() {

			@Override
			public int compare(Jogador j1, Jogador j2) {
				return -j1.getPontos().compareTo(j2.getPontos());
			}
		});
		
		for(int i=0; i<4; i++){
			Text t = new Text(280, eixoY, this.fonteRanking, 
					"Nome: " +jogadores.get(i).getNome() +" - Pontos: "+ jogadores.get(i).getPontos());
